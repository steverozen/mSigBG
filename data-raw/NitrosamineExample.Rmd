...
spectra <- # The input spectra for one nitrosamine
  nitrosamine.examples$catSBS96[ , in.ranges[[params$whichnitro]]]
...
# @param bg.catalog A matrix-like object in which each column is
#   represents one sample and each row is a mutation type, e.g.
#   CAG -> CTG. The counts in each cell are the number of mutations
#   due the background mutational signature. In the future, 
#   rewrite this so this parameter is called "partial.spectra.1"
#
# @param target.catalog A matrix-like object with the same
# dimensions as bg.catalog ***** rename to "partial.spectra.2"
...
```{r}
ret$all.opt.ret$message
```

```{r}
ret$all.opt.ret$status
```
...
### Inferred `r params$whichnitro` count signature the HepG2 background signature 
...
#### Inferred `r params$whichnitro` density signature and the HepG2 background signature 
...
P values versus the null that the spectra
can be reconstructed reasonably well without the inferred target
signature.

```{r, sig.pres.sig.p.values}
...
P values versus the null that the spectra
can be reconstructed reasonably well without the background
signature.

```{r, sig.pres.bg.p.values}
lapply(retval, function(x) x$chisq.p)
```