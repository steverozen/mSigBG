...
if (!file.exists("hepg2.cis.sep.Rdata")) {
  hepg2.cis.sep <- one.separation("hepg2.cis", hepg2.cis, background.info[["HepG2"]])
  save(hepg2.cis.sep, file = "hepg2.cis.sep.Rdata")
} else {
  load("hepg2.cis.sep.Rdata")
}

# Are the inferred target signatures by method 1 similar across the 4 mcfa1 spectra?
if (!file.exists("mcf10a.cis.sep.Rdata")) {
  mcf10a.cis.sep <- one.separation("mcf10a.cis", mcf10a.cis, background.info[["MCF10A"]])
  save(mcf10a.cis.sep, file = "mcf10a.cis.sep.Rdata")
} else {
  load("mcf10a.cis.sep.Rdata")
}
...
b.x0 <- 0.1 * colSums(spectra)
est.target.sig.and.b.x0 <- c(sig0, b.x0)
upper.bounds.of.target.sig <- rep(1, nrow(spectra))
m.opts <- SeparateSignatureFromBackgroundOptions()

ret.lop <- nloptr::nloptr(
...
```{r}
length(ret.lop$solution[1:nrow(spectra)])


length(ret.lop$solution[(1 + nrow(spectra)):length(ret.lop$solution)])
```
...
##### MCF10A-Cisplatinum
```{r}
ts.mcf10a.cis <- target_sig_M1(mcf10a.cis, mcf10a.cis.sep)
target.sig.method1 <- ts.mcf10a.cis/colSums(ts.mcf10a.cis)
```
...
for (i in 1:(ncol(ts.hepg2.cis)-1)){
  for (j in (i+1):ncol(ts.hepg2.cis)){
    ...
    colnames <- c(colnames, paste(colnames(ts.hepg2.cis)[i], colnames(ts.hepg2.cis)[j], sep=" VS "))
...
Are the target signature similar in the two cell line?
```{r}
# when comparing the target signature of the two cell line --> the similarity is higher
philentropy::cosine_dist(hepg2.cis.sep$inferred.target.sig.as.catalog,
                         mcf10a.cis.sep$inferred.target.sig.as.catalog, testNA = TRUE)
```

```{r}
subcolor_sim(hepg2.cis.sep$inferred.target.sig.as.catalog,
             mcf10a.cis.sep$inferred.target.sig.as.catalog)
```

```{r, fig.height=3, fig.width=7}
pcat(cbind(hepg2.cis.sep$inferred.target.sig.as.catalog,
           mcf10a.cis.sep$inferred.target.sig.as.catalog))
```
...
Could the target signature contains an enhanced intrinsic signature?
cosine similarity between subcolor of background and target
```{r}
