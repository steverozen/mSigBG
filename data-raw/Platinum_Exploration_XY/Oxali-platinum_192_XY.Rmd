...
hepg2.oxa_vcfs <- list.files(path = "../data/VCFs/HepG2_oxab/" , full.names = TRUE)
mcf10a.oxa_vcfs <- list.files(path = "../data/VCFs/MCF10A_oxab/" , full.names = TRUE)
...
if (!file.exists("../data/hepg2.oxa.sep.Rdata")) {
  hepg2.oxa.sep <- one.separation("hepg2.oxa", hepg2.oxa.spec, bg.hepg2.192)
  save(hepg2.oxa.sep, file = "../data/hepg2.oxa.sep.Rdata")
} else {
  load("../data/hepg2.oxa.sep.Rdata")
}
```


```{r}
if (!file.exists("../data/mcf10a.oxa.sep.Rdata")) {
  mcf10a.oxa.sep <- one.separation("mcf10a.oxa", mcf10a.oxa.spec, bg.mcf10a.192)
  save(mcf10a.oxa.sep, file = "../data/mcf10a.oxa.sep.Rdata")
} else {
  load("../data/mcf10a.oxa.sep.Rdata")
}
```
...
```{r, fig.height=2, fig.width=7}
# hepg2.oxa.sep is obtained from one_separation using bg.hepg2.192
pcat(hepg2.oxa.sep$inferred.target.sig.as.catalog)
```
...
```{r}
ts.hepg2.oxa <- target_sig_M1(hepg2.oxa.spec, hepg2.oxa.sep)
ts.mcf10a.oxa <- target_sig_M1(mcf10a.oxa.spec, mcf10a.oxa.sep)
```

```{r, fig.height=21, fig.width=10}
pcat(cbind(ts.hepg2.oxa,ts.mcf10a.oxa))
```

# Pairwise comparison for the oxaplatin
comparisons within HepG2-oxaplatin
```{r}
colnames <- c()
pw.subsim = matrix(data=NA, nrow=6, ncol=0)
x=1
for (i in 1:(ncol(ts.hepg2.oxa)-1)){
  for (j in (i+1):ncol(ts.hepg2.oxa)){
    tmp.subsim <- subcolor_sim(ts.hepg2.oxa[,i,drop=FALSE],ts.hepg2.oxa[,j,drop=FALSE])[["by.color"]]
    colnames <- c(colnames, paste(sub("_hg38_SNVintersect", "", colnames(ts.hepg2.oxa)[i], fixed = TRUE),
                                  sub("_hg38_SNVintersect", "", colnames(ts.hepg2.oxa)[j], fixed = TRUE),
                                  sep="-"))
...
comparisons within MCF10A-oxaplatin
```{r}
colnames <- c()
pw.subsim = matrix(data=NA, nrow=6, ncol=0)
x=1
for (i in 1:(ncol(ts.mcf10a.oxa)-1)){
  for (j in (i+1):ncol(ts.mcf10a.oxa)){
    tmp.subsim <- subcolor_sim(ts.mcf10a.oxa[,i,drop=FALSE],ts.mcf10a.oxa[,j,drop=FALSE])[["by.color"]]
    colnames <- c(colnames, paste(sub("_hg38_SNVintersect", "", colnames(ts.mcf10a.oxa)[i], fixed = TRUE),
                                  sub("_hg38_SNVintersect", "", colnames(ts.mcf10a.oxa)[j], fixed = TRUE),
                                  sep="-"))
    pw.subsim <- cbind(pw.subsim, tmp.subsim)
  }
}
...
legend("bottomright", legend = colnames(pw.subsim), col=1:ncol(pw.subsim), pch=1, cex = 0.4,ncol=3)
```


comparisons across HepG2 & MCF10A oxaplatin
```{r}

colnames <- c()
pw.subsim = matrix(data=NA, nrow=6, ncol=0)
x=1
for (i in 1:(ncol(ts.hepg2.oxa))){
  for (j in 1:ncol(ts.mcf10a.oxa)){
    tmp.subsim <- subcolor_sim(ts.hepg2.oxa[,i,drop=FALSE],ts.mcf10a.oxa[,j,drop=FALSE])[["by.color"]]
    colnames <- c(colnames, paste(sub("_hg38_SNVintersect", "", colnames(ts.hepg2.oxa)[i], fixed = TRUE),
                                  sub("_hg38_SNVintersect", "", colnames(ts.mcf10a.oxa)[j], fixed = TRUE),
                                  sep="-"))
    pw.subsim <- cbind(pw.subsim, tmp.subsim)
...
```{r}
matplot(pw.subsim, type = "b",pch=1,col = 1:ncol(pw.subsim), xlab = "1.C>A   2.C>G   3.C>T   4.T>A   5.T>C   6.T>G")
legend("bottomright", legend = colnames(pw.subsim), col=1:ncol(pw.subsim), pch=1, cex = 0.4, ncol=5)
```

comparisons within HepG2-oxaplatin (weighted)
```{r}
pw.subsim <- c()
for (i in 1:(ncol(ts.hepg2.oxa)-1)){
  for (j in (i+1):ncol(ts.hepg2.oxa)){
    tmp.subsim <- subcolor_sim(ts.hepg2.oxa[,i,drop=FALSE],ts.hepg2.oxa[,j,drop=FALSE])[["weighted.avg"]]
    names(tmp.subsim) <- paste(sub("_hg38_SNVintersect", "", colnames(ts.hepg2.oxa)[i], fixed = TRUE),
                                  sub("_hg38_SNVintersect", "", colnames(ts.hepg2.oxa)[j], fixed = TRUE),
                                  sep="-")
...
axis(1, at=1:length(pw.subsim), labels = gsub("HepG2_Oxa_", "", names(pw.subsim), fixed = TRUE))
```


comparisons within mcf10a-oxaplatin (weighted)
```{r}
pw.subsim <- c()
for (i in 1:(ncol(ts.mcf10a.oxa)-1)){
  for (j in (i+1):ncol(ts.mcf10a.oxa)){
    tmp.subsim <- subcolor_sim(ts.mcf10a.oxa[,i,drop=FALSE],ts.mcf10a.oxa[,j,drop=FALSE])[["weighted.avg"]]
    names(tmp.subsim) <- paste(sub("_hg38_SNVintersect", "", colnames(ts.mcf10a.oxa)[i], fixed = TRUE),
                                  sub("_hg38_SNVintersect", "", colnames(ts.mcf10a.oxa)[j], fixed = TRUE),
                                  sep="-")
    pw.subsim <- c(pw.subsim, tmp.subsim)
  }
...
axis(1, at=1:length(pw.subsim), labels = gsub("MCF10A_Oxa_", "", names(pw.subsim), fixed = TRUE))
```

comparisons within HepG2 VS Mcf10a oxaplatin (weighted)
```{r}
pw.subsim <- c()
for (i in 1:ncol(ts.hepg2.oxa)){
  for (j in 1:ncol(ts.mcf10a.oxa)){
    tmp.subsim <- subcolor_sim(ts.hepg2.oxa[,i,drop=FALSE],ts.mcf10a.oxa[,j,drop=FALSE])[["weighted.avg"]]
    names(tmp.subsim) <- paste(sub("_hg38_SNVintersect", "", colnames(ts.hepg2.oxa)[i], fixed = TRUE),
                                  sub("_hg38_SNVintersect", "", colnames(ts.mcf10a.oxa)[j], fixed = TRUE),
                                  sep="-")
    pw.subsim <- c(pw.subsim, tmp.subsim)
...
```{r}
xname <- sub("HepG2_Oxa_", "h", names(pw.subsim), fixed = TRUE)
xname <-sub("MCF10A_Oxa_", "m" , xname, fixed = TRUE)
