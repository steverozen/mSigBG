...
muts_summary_ddply <- ddply(sub.summary,c("Sample.Name"),summarise,
                            NChild=length(sub_num),
                            mean=mean(sub_num),
                            sd=sd(sub_num),
                            se=sd/sqrt(NChild))
...
for(j in 1:10000){
  sel_idx <- sample(1:dim(muts_control)[1],2,replace=T) # sample 2 from all the controls 
...
for(i in 1:dim(a)[1]){
  print(i)
  pvalue <- round(length(which(sel_parents_mean_all_2>round(a[i,"mean"])))
                  /length(sel_parents_mean_all_2),digits = 4)
