# Xueqing's Code Regarding BG sep

setwd("/home/e0261891/project_folder/mSigBG/data-raw/Xueqing_Code")
source("Header.R")
...
for(i in 1:dim(compoundlist)[1]){
  ...
  centroid_compound <- rowMeans(chosen_compound_all) # mean/estimated mutation profile of current compound
...
Sample_withSig_SNR <- read.table("Fig2A.txt",sep = "\t",header = T, as.is = T, quote="\"")
...
compoundlist <- data.frame(table(muts_compound$Sample.Name))
names(compoundlist) <- c("Sample.Name","Freq")
muttype_freq_template <- read.table("MutationType_template.txt", sep = "\t", header = T, as.is = T)
stability_all <- NULL
...
for(i in 1:dim(compoundlist)[1]){
  ...
  # get the spectra of the current compound from sub_catalogue                                                                      
